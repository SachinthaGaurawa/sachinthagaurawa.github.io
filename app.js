...
let challenge = { a: 8, b: 4, op: '+' };

function renderQuestion() {
  const ops = ['+', '-', '×'];
  const op = ops[Math.floor(Math.random() * ops.length)];
  let a = Math.floor(2 + Math.random() * 9);
  let b = Math.floor(2 + Math.random() * 9);
  if (op === '-' && b > a) [a, b] = [b, a];
  challenge = { a, b, op };
...
async function startFlow() {
  const entered = Number(answerEl.value.trim());
  const correct = calcAnswer(challenge);
  if (!Number.isFinite(entered) || entered !== correct) {
  ...
  messageEl.textContent = 'Creating secure session...';
  ...
  if (!res.ok) {
    ...
    messageEl.textContent = 'Verification failed on server.';
    return;
  }
  ...
  statusPill.textContent = 'Opened';
  messageEl.textContent = 'Opened in a secure new tab.';
}

verifyBtn.addEventListener('click', startFlow);
previewBtn.addEventListener('click', () => {
  messageEl.textContent = 'Preview mode: the real redirect is protected by server-issued tokens.';
