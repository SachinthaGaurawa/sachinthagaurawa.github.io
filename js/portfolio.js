...
document.addEventListener('DOMContentLoaded', function() {
  ...
  initializeBackToTop();
  initializeAwardsConfetti();
  initializeEnterKeyVerification();
  ...
  form.addEventListener('submit', function(e) {
    ...
    emailjs
      ...
      .then(() => {
        ...
        // Track the lead analytics
        const name = this.querySelector('[name="user_name"]')?.value || 'Unknown';
        const email = this.querySelector('[name="user_email"]')?.value || 'Unknown';
...
// Download and Verification setup
function initializeDownloadVerification() {
  ...
  if (verifyBtn && !verifyBtn.dataset.bound) {
    ...
    verifyBtn.addEventListener('click', verifyCaptchaAndDownload);
  }
}
...
async function triggerDownload(type) {
  ...
  try {
    ...
    window.__trackAnalytics?.trackDownload(filename);
...
// Awards Confetti
function initializeAwardsConfetti() {
  ...
  cards.forEach(card => {
    ...
    const observer = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && !canvas.dataset.fired) {
        ...
        media.dispatchEvent(new Event('mouseenter'));
        setTimeout(() => media.dispatchEvent(new Event('mouseleave')), 2500);
      }
    }, { threshold: 0.4 });
...
// Enter Key Verification for Modal
function initializeEnterKeyVerification() {
